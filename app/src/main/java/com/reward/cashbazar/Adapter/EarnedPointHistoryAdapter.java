...
public class EarnedPointHistoryAdapter extends RecyclerView.Adapter<EarnedPointHistoryAdapter.SavedHolder> {
    ...
    public EarnedPointHistoryAdapter(List<Wallet_ListMenu> list, Context context, String type) {
    ...
    public void onBindViewHolder(@NonNull SavedHolder holder, final int position) {
        try {
            ...
            String image;
            if (type.equals(POC_Constants.HistoryType.WATCH_VIDEO)) {
                image = listPointHistory.get(position).getIcon();
            } else {
                image = type.equals(POC_Constants.HistoryType.TASK) ? listPointHistory.get(position).getIcone() : listPointHistory.get(position).getImage();
            }
            if (image != null) {
...
//                                     holder.ivIcon.setBackground(context.getResources().getDrawable(R.drawable.rectangle_white));
            ...
            if (!type.equals(POC_Constants.HistoryType.SCRATCH_CARD) &&
                    ...
                    !type.equals(POC_Constants.HistoryType.DAILY_LOGIN) &&
                    !type.equals(POC_Constants.HistoryType.REFER_POINT) &&
                    !type.equals(POC_Constants.HistoryType.GIVE_AWAY) &&
                    !type.equals(POC_Constants.HistoryType.WATCH_VIDEO) &&
                    !type.equals(POC_Constants.HistoryType.TYPE_TEXT_TYPING) &&
                    !type.equals(POC_Constants.HistoryType.MILESTONES) &&
                    !type.equals(POC_Constants.HistoryType.WORD_SORTING) &&
//                    !type.equals(POC_Constants.HistoryType.CPXSurvey) &&
                    !type.equals(POC_Constants.HistoryType.MINE_SWEEPER) &&
                    !type.equals(POC_Constants.HistoryType.SPIN) &&
                    !type.equals(POC_Constants.HistoryType.ALPHABET_GAME) &&
            ...
            if (listPointHistory.get(position).getEntryDate() != null) {
                ...
                } else if (type.equals(POC_Constants.HistoryType.TYPE_TEXT_TYPING)) {
                    holder.txtDate.setText(Common_Utils.modifyDateLayout(listPointHistory.get(position).getUpdateDate()));
                } else {
            ...
            } else if (type.equals(POC_Constants.HistoryType.TYPE_TEXT_TYPING) && listPointHistory.get(position).getWinningPoints() != null) {


                holder.txtPoint.setText("+" + listPointHistory.get(position).getWinningPoints());
//                AppLogger.getInstance().d("TAGahtrrth", "qph5y" + listPointHistory.get(position).getWinningPoints());
                holder.lblPoints.setText(Integer.parseInt(listPointHistory.get(position).getWinningPoints()) > 1 ? "Bucks" : "Buck");
            } else if (listPointHistory.get(position).getPoints() != null) {

//                AppLogger.getInstance().d("TAGargerea", "gggggggeauilt" + listPointHistory.get(position).getPoints());
                if (listPointHistory.get(position).getType() != null) {
                    if (listPointHistory.get(position).getType().matches("1")) {
                ...
                holder.lblPoints.setText("Buck");
            }
            if ((listPointHistory.size() - 1) == position) {
    ...
    private void loadAppLovinNativeAds(FrameLayout adLayout) {
        try {
//            AppLogger.getInstance().e("TAG", "======++" + "loadad");
...
//                    AppLogger.getInstance().e("TAG", "======++" + nativeAdLoader);
                }

                @Override
                public void onNativeAdLoadFailed(final String adUnitId, final MaxError error) {
                    adLayout.setVisibility(View.GONE);
//                    AppLogger.getInstance().e("TAG", "======++" + error);
                }

                @Override
                public void onNativeAdClicked(final MaxAd ad) {
//                    AppLogger.getInstance().e("TAG", "======++" + ad);
                }
            });
