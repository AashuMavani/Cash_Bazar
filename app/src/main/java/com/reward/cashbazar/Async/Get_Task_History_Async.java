...
public class Get_Task_History_Async {
    private Activity activity;
    private JSONObject jObject;
    private AES_Cipher pocAesCipher;

    public Get_Task_History_Async(final Activity activity, String taskId) {
        ...
        try {
            ...
            jObject.put("POHGRW", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userId));
            jObject.put("XW31WW", POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN());
            jObject.put("QXG4FV", taskId);

            jObject.put("QR1W5H", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("IWBWWG", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AdID));
            jObject.put("EJGH3G", Build.MODEL);
            jObject.put("TYIKDTY", Build.VERSION.RELEASE);
            jObject.put("G80VT3", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AppVersion));
            jObject.put("2Y2T2H", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.totalOpen));
            jObject.put("M8HKTQ", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.todayOpen));
            jObject.put("46NGQP", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
...
//            AppLogger.getInstance().e("Get TASK OFFER LIST ORIGINAL ==>", jObject.toString());
            Call<Api_Response> call = apiService.getTaskDetails(POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN(), String.valueOf(n), jObject.toString());
    ...
    private void onPostExecute(Api_Response response) {
        try {
            Common_Utils.dismissProgressLoader();
            Task_History_Response_Model responseModel = new Gson().fromJson(new String(pocAesCipher.decrypt(response.getEncrypt())), Task_History_Response_Model.class);
            ...
            if (responseModel.getStatus().equals(POC_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(POC_Constants.STATUS_SUCCESS)) {
                    if (activity instanceof TaskDetailsActivity) {
                        ((TaskDetailsActivity) activity).setData(responseModel);
                    }
