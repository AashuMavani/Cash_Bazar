...
public class Async_Minesweeper_save {
    private Activity activity;
    private JSONObject jObject;
    private AES_Cipher cipher;

    public Async_Minesweeper_save(final Activity activity, String points) {
        this.activity = activity;
        cipher = new AES_Cipher();
        try {
            Common_Utils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("TUKJLJKLGHGHC", points);
            jObject.put("DFGFD", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userId));
            jObject.put("IOJKHHJYHJUY", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken));
            jObject.put("QYRHNGHJWSXC", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AdID));
            jObject.put("ERFHHNK", Build.MODEL);
            jObject.put("IOIUOU", POC_SharePrefs.getInstance().getString(POC_SharePrefs.FCMregId));
            jObject.put("SDFSDF", Build.VERSION.RELEASE);
            jObject.put("VBNVBN", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AppVersion));
            jObject.put("FDGDGD", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.totalOpen));
            jObject.put("DFGDFG", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.todayOpen));
            jObject.put("ASDASD", Common_Utils.verifyInstallerId(activity));
            jObject.put("QWSERS", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
...
//            Logger_App.getInstance().e("Save mine ORIGINAL ==>", jObject.toString());
//            Logger_App.getInstance().e("Save mine ENCRYPTED ==>", AESCipher.encrypt(BuildConfig.MKEY, BuildConfig.MIV, jObject.toString()));

            Call<Api_Response> call = apiService.saveMinesweeper(POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken), String.valueOf(n), cipher.bytesToHex(cipher.encrypt(jObject.toString())));
...
//            Logger_App.getInstance().e("RESPONSE", "" + new Gson().toJson(responseModel));
            if (responseModel.getStatus().equals(POC_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(POC_Constants.STATUS_SUCCESS)) {
                    ((Bombsweeper_Activity) activity).updateDataChanges(responseModel);
