...
public class Get_Scratch_Ticket_List_Async {
    private Activity activity;
    private JSONObject jObject;
    private AES_Cipher pocAesCipher;

    public Get_Scratch_Ticket_List_Async(final Activity activity) {
        ...
        try {
            ...
            jObject.put("RJHYWK", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userId));
            jObject.put("OCS2ME", POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN());
            jObject.put("OB0W2O", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("PKHS3W", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AdID));
            jObject.put("DV4LT7", Build.MODEL);
                jObject.put("FVTHSD", Build.VERSION.RELEASE);
            jObject.put("EKG3FU", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AppVersion));
            jObject.put("2TWRV6", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.totalOpen));
            jObject.put("HUIDQJ", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.todayOpen));
            jObject.put("OB0W2O", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<Api_Response> call = apiService.getScratchcard(POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(POC_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(POC_Constants.STATUS_SUCCESS)) {
                    if (activity instanceof Activity_Scratch_CouponsGame) {
                        ((Activity_Scratch_CouponsGame) activity).setData(responseModel);
                    }
