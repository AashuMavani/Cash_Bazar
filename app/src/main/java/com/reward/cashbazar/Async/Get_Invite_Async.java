...
import com.reward.cashbazar.Activity.ReferUserActivity;
...
public class Get_Invite_Async {
    private Activity activity;
    private JSONObject jObject;
    private AES_Cipher pocAesCipher;

    public Get_Invite_Async(final Activity activity) {
        ...
        try {
            ...
            jObject.put("KO9JHW", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userId));
            jObject.put("CPNEUT", POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN());
            jObject.put("IWHWWD", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("K4UUQG", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AdID));
            jObject.put("T9BZGM", Build.MODEL);
            jObject.put("ASDGHBFAD", Build.VERSION.RELEASE);
            jObject.put("QK7OJ7", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AppVersion));
            jObject.put("0OFDEN", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.totalOpen));
            jObject.put("UZ3X3R", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.todayOpen));
            jObject.put("CPNEUT", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
...
//            //AppLogger.getInstance().e("Get Invite ENCRYPTED ==>", AES_Cipher.encrypt(POC_Constants.getMKEY(),POC_Constants.getMIV(), jObject.toString()));
            Call<Api_Response> call = apiService.getInviteAsync(POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN(), String.valueOf(n), jObject.toString());
    ...
    private void onPostExecute(Api_Response response) {
        try {
            Common_Utils.dismissProgressLoader();
            Invite_Response_Model responseModel = new Gson().fromJson(new String(pocAesCipher.decrypt(response.getEncrypt())), Invite_Response_Model.class);
            ...
            if (responseModel.getStatus().equals(POC_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(POC_Constants.STATUS_SUCCESS)) {
                    if (activity instanceof ReferUserActivity) {
//                        ((MainActivity) activity).onInviteDataChanged(responseModel);
                        ((ReferUserActivity) activity).setData(responseModel);
                    }
