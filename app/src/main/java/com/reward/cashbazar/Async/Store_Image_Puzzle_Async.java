...
public class Store_Image_Puzzle_Async {
    private Activity activity;
    private JSONObject jObject;
   private AES_Cipher pocAesCipher;


    public Store_Image_Puzzle_Async(final Activity activity, String point, String puzzleId) {
        ...
        try {
            ...
            jObject.put("UT3IAO", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userId));
            jObject.put("3TDYZ1",POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN());
            jObject.put("9TGEPL", point);
            jObject.put("C2IMGW", puzzleId);
            jObject.put("L4V5F0", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.totalOpen));
            jObject.put("ODQTGJ", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.todayOpen));
            jObject.put("25Z1A5", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AdID));
            jObject.put("QY36KS", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AppVersion));
            jObject.put("JHKFJH", Common_Utils.verifyInstallerId(activity));
            jObject.put("S1HFR9", Build.MODEL);
            jObject.put("BAS2IW", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("BOBWD7", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<Api_Response> call = apiService.saveImagePuzzleData(POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(POC_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(POC_Constants.STATUS_SUCCESS)) {
                    ((Picture_PuzzleQuizActivity) activity).onImagePuzzleDataChanged(responseModel);
