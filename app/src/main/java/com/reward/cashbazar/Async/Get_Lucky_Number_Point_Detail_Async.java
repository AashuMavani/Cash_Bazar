...
import com.reward.cashbazar.Activity.LuckyNumberConsentHistoryActivity;
...
public class Get_Lucky_Number_Point_Detail_Async {
   ...
   private AES_Cipher pocAesCipher;
    ...
    public Get_Lucky_Number_Point_Detail_Async(final Activity activity, String type, String pageNo) {
        ...
        try {
            ...
            jObject.put("U7W7QX", POC_SharePrefs.getInstance().getString(POC_SharePrefs.userId));
            jObject.put("I3VKBM", POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN());
            jObject.put("PGILAQ", pageNo);
            jObject.put("OWDXDY", type);
            jObject.put("TWBKPD", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("C97ZRA", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AdID));
            jObject.put("GAXPSG", Build.MODEL);
            jObject.put("FUHIF", Build.VERSION.RELEASE);
            jObject.put("WG03OH", POC_SharePrefs.getInstance().getString(POC_SharePrefs.AppVersion));
            jObject.put("9HFAHH", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.totalOpen));
            jObject.put("3WOTDK", POC_SharePrefs.getInstance().getInt(POC_SharePrefs.todayOpen));
            jObject.put("TOXGW4", Common_Utils.verifyInstallerId(activity));
            jObject.put("9C000R", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<Api_Response> call = apiService.getLuckyNumberHistory(POC_SharePrefs.getInstance().getBoolean(POC_SharePrefs.IS_LOGIN) ? POC_SharePrefs.getInstance().getString(POC_SharePrefs.userToken) : POC_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(POC_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(POC_Constants.STATUS_SUCCESS)) {
                    ((LuckyNumberConsentHistoryActivity) activity).setData(type, responseModel);
