...
import com.reward.cashbazar.Adapter.Quiz_My_Detail_Adapter;
import com.reward.cashbazar.Async.Get_Quiz_Point_Detail_Async;
...
public class My_Quiz_Detail_Fragment extends Fragment {
    private View view;
    ...
    private final List<Quiz_My_Detail_Item> listPointHistory = new ArrayList<>();
    ...
    public My_Quiz_Detail_Fragment() {
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rvHistoryList.setAdapter(new Quiz_My_Detail_Adapter(listPointHistory, getActivity()));
        ...
        nestedScrollView.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() {
            ...
            public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                if (scrollY == v.getChildAt(0).getMeasuredHeight() - v.getMeasuredHeight()) {
                    if (pageNo < numOfPage) {
                        new Get_Quiz_Point_Detail_Async(getActivity(), POC_Constants.HistoryType.QUIZ_MY_CONTEST, String.valueOf(pageNo + 1));
                    }
                }
            }
        });
        new Get_Quiz_Point_Detail_Async(getActivity(), POC_Constants.HistoryType.QUIZ_MY_CONTEST, String.valueOf(pageNo));
    }

    public void setData(Point_History_Model responseModel) {
        if (responseModel != null && responseModel.getQuizMyHistoryList() != null && responseModel.getQuizMyHistoryList().size() > 0) {
            ...
            listPointHistory.addAll(responseModel.getQuizMyHistoryList());
            if (prevItemCount == 0) {
                rvHistoryList.getAdapter().notifyDataSetChanged();
            } else {
                rvHistoryList.getAdapter().notifyItemRangeInserted(prevItemCount, responseModel.getQuizMyHistoryList().size());
            }
