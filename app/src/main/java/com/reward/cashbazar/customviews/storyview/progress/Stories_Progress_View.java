package com.reward.cashbazar.customviews.storyview.progress;
...
public class Stories_Progress_View extends LinearLayout {
    ...
    private static final String TAG = Stories_Progress_View.class.getSimpleName();
    ...
    private final List<Pausable_ProgressBar> progressBars = new ArrayList<>();
    ...
    public Stories_Progress_View(Context context) {
        this(context, null);
    }

    public Stories_Progress_View(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public Stories_Progress_View(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public Stories_Progress_View(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void bindViews() {
        ...
        for (int i = 0; i < storiesCount; i++) {
            final Pausable_ProgressBar p = createProgressBar();
            progressBars.add(p);
            addView(p);
            if ((i + 1) < storiesCount) {
                addView(createSpace());
    ...
    private Pausable_ProgressBar createProgressBar() {
        Pausable_ProgressBar p = new Pausable_ProgressBar(getContext());
    ...
    public void skip() {
        ...
        if (current < 0) return;
        Pausable_ProgressBar p = progressBars.get(current);
        isSkipStart = true;
        p.setMax();
    }
    ...
    private Pausable_ProgressBar.Callback callback(final int index) {
        return new Pausable_ProgressBar.Callback() {
            @Override
            ...
            public void onFinishProgress() {
                if (isReverseStart) {
                    ...
                    if (0 <= (current - 1)) {
                        Pausable_ProgressBar p = progressBars.get(current - 1);
     ...
     */
    public void destroy() {
        for (Pausable_ProgressBar p : progressBars) {
            p.clear();
        }
    }
